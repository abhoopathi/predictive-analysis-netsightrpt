...
#def main(day):
    ...
    logging.basicConfig(filename=config.proj_path+'/log_for_demo/p8/p8.log',level=logging.INFO)
    ...
    if(real_flag==1):
        ...
        prophet_df.to_sql(con=engine, name='valid_nofapps_per_source', if_exists='replace', index=False )
        app_count_full_df.to_sql(con=engine, name='data_nofapps_per_source', if_exists='replace', index=False )

    ## checking whether forecast and anlyse table avilable or not
    with conn.cursor() as cursor:
        stmt = "SHOW TABLES LIKE 'forecast_nofapps_per_source'"
        cursor.execute(stmt)
    ...
    with conn.cursor() as cursor:
        stmt = "SHOW TABLES LIKE 'analyse_nofapps_per_source'"
        cursor.execute(stmt)
        result = cursor.fetchone()
    if result:
    ...
    if(table_present_forecast_flag ==1):
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "alter table forecast_nofapps_per_source add partition (partition new values in ('"+str(this_day)+"'))" 
            cursor.execute(sql)
    ...
    if((real_flag ==1) & (config.override_flag_forecast==1)):
        prophet_future_df.to_sql(con=engine, name='forecast_nofapps_per_source', if_exists='replace', index=False )
        table_present_forecast_flag = 0
    elif((real_flag ==1) & (config.override_flag_forecast==0)):
        prophet_future_df.to_sql(con=engine, name='forecast_nofapps_per_source', if_exists='append', index=False )
    ...
    if(table_present_analyse_flag == 1):
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = " alter table analyse_nofapps_per_source remove partitioning" 
            cursor.execute(sql)
    ...
    if(config.override_flag_analysis == 1):
        prophet_analysis_df.to_sql(con=engine, name='analyse_nofapps_per_source', if_exists='replace',index=False)
        table_present_analyse_flag = 0
    else:
        prophet_analysis_df.to_sql(con=engine, name='analyse_nofapps_per_source', if_exists='append',index=False)
    ...
    if(table_present_analyse_flag == 1):
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "select distinct run_date as run_date from analyse_nofapps_per_source " 
            cursor.execute(sql)
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "ALTER TABLE analyse_nofapps_per_source PARTITION BY LIST COLUMNS (run_date) ( PARTITION latest VALUES IN ("+dt_str+") );" 
            cursor.execute(sql)
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "ALTER TABLE analyse_nofapps_per_source PARTITION BY LIST COLUMNS (run_date) ( PARTITION latest VALUES IN ('"+str(this_day)+"') );" 
            cursor.execute(sql)
    ...
    if(table_present_forecast_flag ==1):
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "alter table forecast_nofapps_per_source truncate partition latest" 
            cursor.execute(sql)
        ...
        with conn.cursor() as cursor:
            # Read a  record
            sql = "alter table forecast_nofapps_per_source drop partition latest" 
            cursor.execute(sql)

        ## to make new as 'latest' partition the inputed table 
        with conn.cursor() as cursor:
            # Read a  record
            sql = "ALTER TABLE forecast_nofapps_per_source REORGANIZE PARTITION new INTO ( PARTITION latest VALUES IN ('"+str(this_day)+"') );" 
            cursor.execute(sql)
    else:
        # adding the partition
        with conn.cursor() as cursor:
            # Read a  record
            sql = "ALTER TABLE forecast_nofapps_per_source PARTITION BY LIST COLUMNS (run_date) ( PARTITION latest VALUES IN ('"+str(this_day)+"') );" 
            cursor.execute(sql)
    ...
    run_time_table = pd.DataFrame({'report':'p2(nofapps_per_source)','run_time':total_real,'run_date':this_day},index=[3])
